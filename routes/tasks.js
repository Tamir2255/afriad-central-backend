...
const { overallTier, canAccessCampaign } = require('../util/tiers');
...
async function getEarnerTier(userId) {
    const socialResult = await db.query(
        `SELECT platform, followers_count, manual_tier, verification_status FROM social_accounts WHERE user_id = $1`,
        [userId]
    );
    return overallTier(socialResult.rows);
}
...
router.post('/verify-video', auth, requireEarner, async (req, res) => {
    ...
    try {
        ...
        const tier = await getEarnerTier(req.user.id);
        if (!canAccessCampaign(tier, campaign.total_units)) {
            await client.query('ROLLBACK');
            return res.status(403).json({ error: "Your current follower tier doesn't qualify for this campaign size yet." });
        }
