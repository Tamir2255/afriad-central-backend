...
// Feed of live campaigns an earner can complete — filtered by niche, targeting scope, and follower tier.
router.get('/available/:taskType', auth, async (req, res) => {
    ...
    try {
        ...
        const tier = overallTier(socialResult.rows);
        if (tier === 'ineligible') {
        ...
        const campaigns = result.rows.filter((c) => canAccessCampaign(tier, c.total_units));
        res.json({ campaigns, tier });
    } catch (err) {
        console.error(err);
